 ...
 */
public class OldFormattedStringDecomposer {
    ...
    public static boolean iterateFormatted(String text, Style defaultStyle, FormattedCharSink sink) {
        // this method is adapted from StringDecomposer::iterateFormatted which normally removes all formatting codes (in the form of '§' + code) and converts them directly to the current style
    ...
    private static class WidthLimitedCharSink implements FormattedCharSink {
        ...
        public boolean accept(int position, Style style, int codePoint) {
            if (position >= this.skip) {
                this.maxWidth -= this.stringSplitter.stringWidth(FormattedCharSequence.forward(Character.toString(codePoint), style));
            }
